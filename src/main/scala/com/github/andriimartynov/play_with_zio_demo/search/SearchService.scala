...
import com.github.andriimartynov.play_with_zio_demo.configuration.{
  AppConfiguration,
  SearchConfiguration
}
...
object SearchService {
  ...
  object Service {
    def http(
      ...
      service: HttpClientService.Service
    ): Service =
      new Service {
        def search(q: String): Task[String] =
          service
            .get(
              s"""https://www.googleapis.com/customsearch/v1?key=${config.apiKey}&cx=${config.searchEngineId}:omuauf_lfve&q=$q"""
            )
      }
  }
  ...
  lazy val http: ZLayer[HasAppConfiguration with HttpClientService, Nothing, SearchService] =
    ZLayer.fromFunction(hasClient =>
      Service.http(hasClient.get[AppConfiguration], hasClient.get[HttpClientService.Service])
    )
